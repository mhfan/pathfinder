...
use pathfinder_canvas::{CanvasRenderingContext2D, LineJoin, Path2D};
use pathfinder_canvas::{TextAlign, TextBaseline, TextMetrics};
use pathfinder_color::{ColorF, ColorU, rgbau, rgbu};
...
pub const WINDOW_HEIGHT: i32 = WINDOW_WIDTH * 3 / 4;
...
fn draw_graph(context: &mut CanvasRenderingContext2D, rect: RectF, time: f32) {
    ...
    let sample_points = samples.iter()
                                                        .enumerate()
                                                        .map(|(index, &sample)| {
...
#[allow(unused)] impl PerfGraph {
    pub fn new(style: GraphStyle, name: &'static str) -> PerfGraph {
    ...
    pub fn render(&self, context: &mut CanvasRenderingContext2D, origin: Vector2F) {
...
impl DemoData {
    pub fn load(resources: &dyn ResourceLoader) -> DemoData {
